...
def get_signals(request):
    ...
    mycursor.execute("SELECT segnalazioni.id, segnalazioni.titolo, segnalazioni.id_allarme, segnalazioni.famiglia_macchina, segnalazioni.sottofamiglia_macchina, segnalazioni.immagine_1, segnalazioni.immagine_2, segnalazioni.immagine_3, occorrenze.titolo, users.username, segnalazioni.created_at, segnalazioni.updated_at FROM segnalazioni INNER JOIN users ON segnalazioni.user_id = users.id INNER JOIN occorrenze ON segnalazioni.id = occorrenze.segnalazione_id")
...
def retrive_user_segnalazioni(request):
    ...
    query = "SELECT segnalazioni.id, segnalazioni.titolo, segnalazioni.id_allarme, segnalazioni.famiglia_macchina, segnalazioni.sottofamiglia_macchina, segnalazioni.immagine_1, segnalazioni.immagine_2, segnalazioni.immagine_3, occorrenze.titolo, users.username, segnalazioni.created_at, segnalazioni.updated_at FROM segnalazioni INNER JOIN users ON segnalazioni.user_id = users.id INNER JOIN occorrenze ON segnalazioni.id = occorrenze.segnalazione_id WHERE segnalazioni.user_id = %s"
...
def retrive_user_soluzioni(request):
    ...
    #user_id = request.user.id
    # user_soluzioni = Soluzioni.objects.filter(user=user_id) koment ke kodi origjinal

    mycursor = mydb.cursor()

    query = "SELECT soluzioni.id, soluzioni.titolo, soluzioni.rank, soluzioni.settore_riferimento, soluzioni.immagine_1, soluzioni.immagine_2, soluzioni.immagine_3, users.username, soluzioni.created_at, soluzioni.updated_at FROM soluzioni INNER JOIN users ON soluzioni.user_id = users.id"
...
def retrive_user_occurrenze(request):
    ...
    query = "SELECT occorrenze.id, occorrenze.titolo, segnalazioni.id_allarme, soluzioni.titolo, occorenze.commessa_macchina, occorrenze.rif_ticket, occorrenze.versione_sw_1, occorrenzze.versione_sw_2, occorrenze.data_occorrenza, users.username, occorrenze.created_at, occorrenze.updated_at FROM occorrenze INNER JOIN users ON occorrenze.user_id = users.id INNER JOIN segnalazioni ON occorrenze.segnalazione_id = segnalazione.id INNER JOIN solutions ON occorrenze.id = soluzioni.occorrenze_id WHERE segnalazioni.user_id = %s"
    ...
    # user_occurrenze = Occorrenze.objects.filter(user=user_id) coment origjinal
